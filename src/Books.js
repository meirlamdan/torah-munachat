export const books = [
  {
    name: 'hebrewbooks',
    src: 'https://hebrewbooks.org/images/hb.bg4.gif',
    about: 'כשישים אלף ספרים סרוקים בקבצי pdf',
    href: 'https://beta.hebrewbooks.org/'
  },
  {
    name: 'אוצר החכמה',
    about: 'מאגר ענק של ספרי קודש סרוקים לקבצי pdf ',
    href: 'https://tablet.otzar.org/'
  },
  {
    name: 'תורת אמת',
    src: require('@/assets/toratemet.png'),
    about: 'מאגר תורני המכיל הרבה ספרי יסוד מוקלדים - להורדה למחשב,קיים גם בגירסת אונליין',
    href: 'http://www.toratemetfreeware.com/'
  },
  {
    name: 'אוצר הספרים היהודי השיתופי',
    src: 'https://wiki.jewishbooks.org.il/mediawiki/images/8/80/JewishBook-logo-%D7%A9%D7%A7%D7%95%D7%A3.png',
    about: 'מאות ספרי קודש מוקלדים בגירסת ויקי ',
    href: 'https://wiki.jewishbooks.org.il/'
  },
  {
    name: 'על התורה',
    about: 'מאות ספרי קודש מוקלדים בממשק נח',
    href: 'https://library.alhatorah.org/'

  },
  {
    name: 'ויקיטקסט-ארון הספרים היהודי',
    src: 'https://upload.wikimedia.org/wikipedia/commons/8/85/Wikisource-logo-he.png',
    about: 'מאות ספרי קודש מוקלדים בגירסת ויקי ',
    href: 'https://he.wikisource.org/wiki/%D7%95%D7%99%D7%A7%D7%99%D7%98%D7%A7%D7%A1%D7%98:%D7%90%D7%A8%D7%95%D7%9F_%D7%94%D7%A1%D7%A4%D7%A8%D7%99%D7%9D_%D7%94%D7%99%D7%94%D7%95%D7%93%D7%99'
  },
  {
    name: 'מקראות גדולות הכתר',
    about: 'מהדורת יסוד מדעית של תנ"ך ‘מקראות גדולות‘.',
    href: 'https://www.mgketer.org/'
  },
  {
    name: 'שיתופתא',
    about: 'מאגר תורני ייחודי מעוצב בצורת הדף המקורית ומערכת לפרסום חידושי תורה על סדר הדף.',
    href: 'https://shitufta.org.il/'
  },
  {
    name: 'תא שמע',
    about: 'מאגר תורני חברתי. (גרסת הבסיס חינם, הכוללת מאגר ספרים מצומצם לשימוש חופשי.)',
    href: 'https://jewishoffice.co.il/%d7%aa%d7%90-%d7%a9%d7%9e%d7%a2/'
  },
  {
    name: 'mercava',
    about: 'מאגר תורני מעוצב בצורה נוחה ללומד.',
    href: 'https://www.themercava.com/'
  },
];
